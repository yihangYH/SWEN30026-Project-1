...
public class Simulation {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Automail automail = new Automail(mailPool, iMailDelivery, num_regular_robots, num_fast_robots, num_bulk_robots,
        		Building.getInstance().getnFloors());
        ...
        feeCalculator.initialize(Building.getInstance().getnFloors());
        while(iMailDelivery.getDeliveredItems().size() != mailGenerator.MAIL_TO_CREATE)
        {
            ...
            try {
				...
				{
					if(feeCalculator.checkDelivered(automail.getRobots()[i]) && apply_fee_Charging) {
						double serviceFee = feeCalculator.updateServiceFee(wifiModem, automail.getRobots()[i]);
						double avgOperatingTime =  feeCalculator.updateMaintenance(automail.getRobots()[i],automail.getRobots());
						automail.getRobots()[i].fee(serviceFee,avgOperatingTime);
					} 
