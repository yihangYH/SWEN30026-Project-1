...
public class ReportDelivery implements IMailDelivery {
    ...
    public void deliver(Robot robot, MailItem deliveryItem, String additionalLog ){
        if(!deliveredItems.contains(deliveryItem))
        {
            ...
            System.out.printf("T: %3d > %7s-> Delivered(%4d) [%s%s]%n", Clock.Time(), robot.getIdTube(), 
    ...
    public void deliver(Robot robot, MailItem deliveryItem, double serviceFee, double avgOperatingTime ) {
    	if(!deliveredItems.contains(deliveryItem))
        {
            deliveredItems.add(deliveryItem);
            double maintenance = robot.getTypeBasedRate() * avgOperatingTime;
            double totalCharge = serviceFee + maintenance;
            System.out.printf("T: %3d > %7s-> Delivered(%4d) [%s | Service Fee: %.2f | Maintenance: %.2f "
            + "| Avg. Operating Time: %.2f | Total Charge: %.2f]%n",
            		Clock.Time(), robot.getIdTube(), deliveredItems.size(), deliveryItem.toString(), 
